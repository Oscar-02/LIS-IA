        ...
        @foreach ($electronics as $electronic)
            <div class="bg-black/10 rounded drop-shadow-2xl">
                <div class="p-5">
                    ...
                    <div class="flex justify-between items-center mt-2">
                        <div class="text-sm border border-dashed rounded p-1">Only <span
                                class="font-bold text-primary">{{ $electronic->stock }} </span>Stock Disponible</div>
                ...
                <form action="{{ route('categories.destroy', $electronic->id) }}" method="POST"
                    ...
                    <a href="{{ route('electronics.show', $electronic->id) }}" class="btn btn-primary rounded p-2">Mostrar</a>
                    @csrf
                    @method('delete')
                    <button class="btn btn-danger rounded p-2" type="submit">Eliminar</button>
                </form>
            </div>
        @endforeach
    </div>
